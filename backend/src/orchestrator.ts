...
ÇIKTI FORMATI: Zengin Türkçe markdown — haber özet tablosu, sentiment değerlendirmesi, katalizör/risk listesi. Minimum 2000 karakter.`;
  ...
  if (PYTHON_QA_REVIEW_ENABLED && agentId === 'qa_review') {
    const outcome = await runPythonQaReview(sessionId, runId, ticker, accumulatedContext);
    return outcome === 'ok' ? 'ok' : 'failed';
  }
...
const AGENT_DEPENDENCIES: Record<string, string[]> = {
  ...
  // U6: knowledge_base artık context_extraction'a değil research_brief'e bağlı.
  // Ticker ceo_output'tan, sub_questions research_brief'ten geliyor. Bu sayede
  // document_evidence context_extraction'dan ÖNCE çalışır ve context_extraction
  // kendisi document_evidence_output'u tüketebilir.
  knowledge_base: ['ceo_output', 'research_brief_output'],
  document_evidence: ['knowledge_base_output', 'research_brief_output'],
  ...
  // U6 evidence-driven integration — 4 target agents receive document_evidence_output.
  context_extraction: ['data_collection_output', 'parse_standardization_output', 'document_evidence_output'],
  financial_analysis: ['parse_standardization_output', 'reconciliation_output', 'context_extraction_output', 'document_evidence_output'],
  ...
  valuation_agent: ['financial_analysis_output', 'context_extraction_output', 'macro_analysis_output', 'document_evidence_output'],
  esg_agent: ['context_extraction_output', 'data_collection_output', 'document_evidence_output'],
