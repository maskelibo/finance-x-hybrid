...
**Required keys:** `agent_id`, `output_id`, `session_id`, `task_id`, `timestamp`, `company`, `analysis_period`, `confidence_overall`, `evidence_refs`, `warnings`, `missing_inputs`, `review_status`
...
Kurallar:
- `agent_id` mutlaka `"financial_analysis"` olmalı (schema `const`).
