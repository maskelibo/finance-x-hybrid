...
**Required keys:** `agent_id`, `output_id`, `session_id`, `task_id`, `timestamp`, `operational_plan`, `quality_checklist`, `confidence_overall`, `warnings`, `review_status`
...
Kurallar:
- `agent_id` mutlaka `"coo"` olmalı (schema `const`).
