...
**Required keys:** `agent_id`, `output_id`, `session_id`, `task_id`, `timestamp`, `company_overview`, `financial_highlights`, `key_findings`, `analytical_limitations`, `mandatory_disclosures`, `confidence_overall`, `review_status`
...
Kurallar:
- `agent_id` mutlaka `"final_summary"` olmalı (schema `const`).
