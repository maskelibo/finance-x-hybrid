...
**Required keys:** `agent_id`, `output_id`, `session_id`, `task_id`, `timestamp`, `company`, `sentiment_summary`, `news_items`, `confidence_overall`, `warnings`, `review_status`
...
Kurallar:
- `agent_id` mutlaka `"sentiment_news_agent"` olmalı (schema `const`).
