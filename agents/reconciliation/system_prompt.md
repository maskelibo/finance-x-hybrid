...
**Required keys:** `agent_id`, `output_id`, `session_id`, `task_id`, `timestamp`, `company`, `reconciled_data`, `discrepancy_report`, `confidence_overall`, `warnings`, `review_status`
...
Kurallar:
- `agent_id` mutlaka `"reconciliation"` olmalı (schema `const`).
