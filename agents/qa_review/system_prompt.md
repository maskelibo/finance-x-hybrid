...
**Required keys:** `agent_id`, `output_id`, `session_id`, `reviewed_output_id`, `producing_agent_id`, `qa_decision`, `dimension_scores`, `quality_flags`, `escalation_recommendation`, `review_timestamp`
...
Kurallar:
- `agent_id` mutlaka `"qa_review"` olmalı (schema `const`).
