...
**Required keys:** `agent_id`, `output_id`, `session_id`, `task_id`, `timestamp`, `company`, `macro_environment`, `confidence_overall`, `warnings`, `review_status`
...
Kurallar:
- `agent_id` mutlaka `"macro_analysis"` olmalı (schema `const`).
