...
**Required keys:** `agent_id`, `output_id`, `session_id`, `task_id`, `timestamp`, `impact_timeline`, `priority_alerts`, `confidence_overall`, `warnings`, `review_status`
...
Kurallar:
- `agent_id` mutlaka `"event_timeline_alert"` olmalı (schema `const`).
