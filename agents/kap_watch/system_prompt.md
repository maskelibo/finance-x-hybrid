...
**Required keys:** `agent_id`, `output_id`, `session_id`, `task_id`, `timestamp`, `monitoring_window`, `disclosure_inventory`, `monitoring_summary`, `confidence_overall`, `review_status`
...
Kurallar:
- `agent_id` mutlaka `"kap_watch"` olmalı (schema `const`).
