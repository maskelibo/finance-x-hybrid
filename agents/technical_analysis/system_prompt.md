...
**Required keys:** `agent_id`, `output_id`, `session_id`, `task_id`, `timestamp`, `company`, `price_trend`, `momentum_indicators`, `technical_summary`, `confidence_overall`, `warnings`, `review_status`
...
Kurallar:
- `agent_id` mutlaka `"technical_analysis"` olmalı (schema `const`).
