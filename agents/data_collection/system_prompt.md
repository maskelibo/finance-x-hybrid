...
**Required keys:** `agent_id`, `output_id`, `session_id`, `task_id`, `timestamp`, `company`, `data_manifest`, `coverage_assessment`, `data_gaps`, `warnings`, `confidence_overall`, `review_status`
...
Kurallar:
- `agent_id` mutlaka `"data_collection"` olmalı (schema `const`).
